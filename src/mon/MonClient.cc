...
bool MonClient::ms_dispatch(Message *m)
{
  ...
  } else if (m->get_connection() != active_con->get_con()) {
...
int MonClient::authenticate(double timeout)
{
  ...
  if (active_con) {
    ldout(cct, 5) << "authenticate success, global_id "
...
void MonClient::handle_auth(MAuthReply *m)
{
  ...
  if (!_hunting()) {
    int ret = active_con->handle_auth(m);
    m->put();
    if (ret != -EAGAIN) {
      _finish_auth(ret);
    }
  ...
  int auth_err = found->second.handle_auth(m);
  m->put();
  if (auth_err == -EAGAIN) {
    return;
  }
  auth = found->second.get_auth();
  assert(auth);
  if (auth_err) {
  ...
  if (!auth_err) {
    ...
    if (session_established_context) {
      std::swap(cb, session_established_context);
    }
    if (cb) {
      monc_lock.Unlock();
      cb->complete(0);
...
MonConnection& MonClient::_add_conn(unsigned rank)
{
  ...
  MonConnection mc(cct, conn, entity_name,
		   get_global_id(), want_keys, rotating_secrets);
...
bool MonClient::ms_handle_reset(Connection *con)
{
  ...
  if (_hunting()) {
    auto found = pending_cons.find(con->get_peer_addr());
    if (found != pending_cons.end()) {
      ldout(cct, 10) << "ms_handle_reset hunted mon " << con->get_peer_addr() << dendl;
    } else {
      ldout(cct, 10) << "ms_handle_reset stray mon " << con->get_peer_addr() << dendl;
    }
    return true;
  } else {
    if (con == active_con->get_con()) {
...
MonConnection::MonConnection(CephContext *cct, ConnectionRef con,
                           ...
                           uint64_t global_id, uint32_t want_keys,
                           RotatingKeyRing *rkeys)
  : cct(cct), con(con),
    entity_name(entity_name), global_id(global_id),
    want_keys(want_keys), rotating_secrets(rkeys)
{}
...
int MonConnection::_authenticate(MAuthReply *m)
{
  assert(auth);
